...
export default function MapContents() {
  ...
  const showErrorMsg = (error: GeolocationPositionError) => {
    switch (error.code) {
      case error.PERMISSION_DENIED:
        setGeoError("Geolocation API의 사용 요청을 거부했습니다.");
        break;
      case error.POSITION_UNAVAILABLE:
        setGeoError("위치 정보를 사용할 수 없습니다.");
        break;
      case error.TIMEOUT:
        setGeoError(
        ...
        break;
      default:
        setGeoError("알 수 없는 오류가 발생했습니다. 관리자에게 문의하세요.");
        break;
    }
  };
  ...
  const filteredData = useMemo(() => {
    const list = (data ?? []) as PublicCapsule[];
    let result = list;

    // viewed
    if (viewed === "UNREAD")
      result = result.filter((d) => d.isViewed === false);
    if (viewed === "READ") result = result.filter((d) => d.isViewed === true);

    // accessible
    if (accessible === "ACCESSIBLE")
      result = result.filter((d) => d.isUnlockable === true);
    if (accessible === "INACCESSIBLE")
      result = result.filter((d) => d.isUnlockable === false);
    ...
    <div className="h-full p-4 lg:p-8 flex flex-col gap-2 lg:gap-4">
      ...
      <div className="flex-1 flex lg:flex-row flex-col gap-4 min-h-0">
        ...
        <div className="relative flex-1 min-h-0 rounded-xl overflow-hidden">
          {geoError ? (
            <div className="w-full h-full absolute z-20 flex items-center justify-center select-none">
              ...
              <div className="absolute text-sm z-3 text-white text-center px-6">
          ...
          <button
            type="button"
            className="cursor-pointer bg-bg absolute bottom-4 right-4 p-3 rounded-xl z-10 shadow-lg text-primary"
          ...
          >
            주변 편지 보기 ({filteredData.length}개)
          </button>
        </div>

        {/* 리스트 (데스크탑만) */}
        <div className="w-90 rounded-xl bg-bg/80 border border-outline hidden lg:flex flex-col gap-3 min-h-0 py-6">
        ...
        <div className="lg:hidden">
          ...
          <div
            className={`fixed left-0 right-0 bottom-0 z-9999 bg-bg rounded-t-2xl border-t border-outline
